...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Zyanyx Chat';
...
export async function selectSession(id, { keepSidebar = false, showLoading = true, immediateLoading = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Zyanyx kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Zyanyx Chat';
    }
